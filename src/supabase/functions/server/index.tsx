...
app.get('/make-server-5053ecf8/meeting-actions', async (c) => {
  try {
...
app.post('/make-server-5053ecf8/meeting-actions', async (c) => {
  try {
    const body = await c.req.json();
    const {
      title,
...
app.put('/make-server-5053ecf8/meeting-actions/:id', async (c) => {
  try {
    const id = c.req.param('id');
    const body = await c.req.json();
...
// Delete meeting action (soft delete)
app.delete('/make-server-5053ecf8/meeting-actions/:id', async (c) => {
  try {
    const id = c.req.param('id');
...
// Get all meetings
app.get('/make-server-5053ecf8/meetings', async (c) => {
  try {
    const { data, error } = await retryWithBackoff(async () => {
      return await supabase
        .from('meetings')
...
// Get single meeting
app.get('/make-server-5053ecf8/meetings/:id', async (c) => {
  try {
    const id = c.req.param('id');
...
// Create/upsert meeting (using Granola meeting ID as key)
app.post('/make-server-5053ecf8/meetings', async (c) => {
  try {
    const body = await c.req.json();
    const {
      id,
...
// Get all sync runs
app.get('/make-server-5053ecf8/sync-runs', async (c) => {
  try {
    const { data, error } = await retryWithBackoff(async () => {
      return await supabase
        .from('sync_runs')
...
// Create sync run
app.post('/make-server-5053ecf8/sync-runs', async (c) => {
  try {
    const body = await c.req.json();
    const {
      started_at,
    ...
    const syncRun = {
      ...
      meetings_fetched: meetings_fetched || 0,
      meetings_processed: meetings_processed || 0,
